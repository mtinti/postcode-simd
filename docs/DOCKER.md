...
`data/cache`, their selected files are placed in `data/sources`, and results and retained
run records go under `results` on the host. No data is baked into the image.
