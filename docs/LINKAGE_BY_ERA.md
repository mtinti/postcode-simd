...
4. **Resolve.** One valid record gives `unique`. Several that agree give `split_consensus`
   with the shared value. Several that disagree give `split_conflict` with a null value. A
   known postcode with no valid record on that day gives `deleted`. An unknown or missing
   postcode gives `not_found`. An event before 1996 gives `no_edition`, with every SIMD column
   null including the matched key.

Nothing chooses a split part for you, averages, or votes.
...
1 = most deprived`. Pass `measure="pw_hb_decile"` or any other SIMD column suffix to change
the measure for every row.
...
To change the measure, edit the six lines of the `CASE` that picks the column.
...
both implementations and asserts the same status, value, matched key and edition for every
event. No patient data is involved; the postcodes come from the directory itself.
...
- **Splits are rare and real.** In this cohort 56 of 1,940 events hit a split postcode, and
  about half of those resolved by consensus. The conflicts are where the two parts of one
  postcode sit in different quintiles; no rule can decide those, only more precise address
  data can.
